#!/usr/bin/env python
"""Batched vLLM anchor miner for Mode-B natural-object canaries.

This is a drop-in faster replacement for `run_modeb_anchor_mining.py`.
It keeps the same output contract (`anchor_records.json`,
`evaluated_candidates.jsonl`, `anchor_records.csv`, summary/contact sheet) but
scores images in batches with vLLM instead of one-image-at-a-time
Transformers/Qwen generation.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser("Mine Mode-B anchors with batched vLLM Qwen-VL scoring.")
    parser.add_argument("--candidate_dirs", nargs="+", required=True)
    parser.add_argument("--output_dir", required=True)
    parser.add_argument("--config", default="configs/semantitrace_flux2_klein_gradient_guided.yaml")
    parser.add_argument("--target_count", type=int, default=250)
...
def build_vllm_inputs(paths: list[Path], prompt: str, processor: Any, args: argparse.Namespace) -> tuple[list[dict[str, Any]], list[Path], list[tuple[int, int]]]:
...
def main() -> None:
    ...
    model_name = cfg.get("models", {}).get("surrogate_vlm", "Qwen/Qwen3-VL-8B-Instruct")
    ...
    idx = 0
    while idx < len(candidates) and len(accepted) < args.target_count:
        batch_paths: list[Path] = []
        while idx < len(candidates) and len(batch_paths) < args.batch_size:
            path = candidates[idx]
            idx += 1
        ...
        if not batch_paths:
            continue
        inputs, kept_paths, sizes = build_vllm_inputs(batch_paths, prompt, processor, args)
        skipped = set(batch_paths) - set(kept_paths)
        for path in skipped:
            row = {"source_image_path": rel(path), "error": "image_open_or_resize_failed"}
        ...
        for path, size, output in zip(kept_paths, sizes, outputs):
            ...
            if parsed is None or parsed["score"] < args.min_score:
                print(f"reject {len(evaluated):04d} score={row['score']:.1f} {path.name} {row['scene_type'][:60]!r}", flush=True)
                continue
